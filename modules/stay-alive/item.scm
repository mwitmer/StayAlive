  ...
  ((initialize! (method ()
   ...
   (put-on 
	...
	(if parts
	    (begin
	      ...
	      (set! (this 'worn?) #t)						      
	      (set! (this 'worn-on) (map (applicator #:op 'reference) parts))
   ...
   (wield 
    (method (level agent)
      (and-let* 
	  ((eligible-parts 
	    ((agent 'body) 'find (pred-and (this #:op 'get '(wield-requires pred)) 
					   (pred-not (applicator 'wielding? #:def #f)))))
	   (parts 
	    (select-part-from-list eligible-parts "with" (this #:op 'get '(wield-requires quantity) #:def 1))))
	(if parts
