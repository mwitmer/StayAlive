...
(define (player-examine-level agent level)
  (level 'describe agent (player-select-square level (agent 'location) "select square")))

(define process-player-command 
  (lambda (level player command)
    (let* ((saved-command (player #:def #f 'saved-command))
           (count (player #:def #f 'count))
           (can-repeat-command? 
	    (lambda (command)
		...
		((update-count terminal inventory pick-up quit reset-count) #f))))
		  ...
		  (if (eq? (cadr command) 'long) (move-agent-direction-long level player (car command))))
                 ((update-count) 
		  (let* ((count (player #:def #f 'count))
			 (new-count (string->number 
		  ...
		  (or
		   ...
		   (let ((melee-target (level 'agent-at (make-new-location (player 'location) command))))
		     (if melee-target
			 (player 'melee-attack melee-target level)))))
                 ((terminal) (terminal the-game-module))
                 ((reset-count) (if count (begin (message "repeat count reset") (set! (player 'count) #f))) #f)
                 ((unknown) (message "I don't know what that means") #f)
		 ((follow-up-command) (player #:def #f 'follow-up-command))
                 ((wait) #t)		
		 ((close-door) 
		  (let ((door-direction (player-select-direction)))
		    (if (not (eq? door-direction 'none))
			(let ((door-location (make-new-location (player 'location) door-direction)))
			 (if ((level (list 'squares door-location)) #:def #f 'open-door?)
			     (if (level 'contains-agent? door-location)
				 (begin (message (format #f "~a is in the way!" 
							 (capitalize 
							  ((level 'agent-at door-location) 'describe)))) #f)
				 (begin
				   ...
				   (level 'set-square! 
					  ...
					  (cadr door-location) 
					  (instance ClosedDoor))
				   (level 'remember (car door-location) (cadr door-location))
				   #t))
			     (begin
			...
			(begin (message "never mind") #f))))
                 ...
                 ((down-stairs) ((level `(squares (,(player 'row) ,(player 'col)))) 'go-down player level))
...
(define-object Player
     ...
     (let ((item (player-select-item (this 'items) "throw what?" "nothing to throw"))
	   ...
	   (and-let* 
		...
		(let ((agent (level 'agent-at current)))
		  (cond 
		   ((and agent (item 'hit-agent level this agent distance)) (item-in-flight item current #t))
		   (((level `(squares (,@current))) 'stops-projectiles?) (item-in-flight item previous #t))
  ...
  (drop (method (level)
	  (let ((item (player-select-item (this 'items) "drop what?" "nothing to drop")))
		...
		(begin
		  ...
		  #t)
		#f))))
  (pick-up (method (level)
	     (let ((item (player-select-item 
			  (filter 
			   (lambda (item) 
			     (receive (row col)
		   ...
		   (begin
		     (message (format #f "picked up ~a (~a)" (item 'describe-short) (item 'letter)))
  ...
  (take-turn (method (paths-to-player player level)
	       ...
	       (let ((command (or (this #:def #f 'saved-command) (get-command))))
		 (clear-message)
