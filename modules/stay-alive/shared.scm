...
(define wrap-remove-item 
  (method (item #:optional level agent) 
    (if (and level agent (item 'wielded? #:def #f)) (item 'unwield level agent))
    (if (and level agent (item 'worn? #:def #f)) (item 'take-off level agent))
    (if agent (message-player level (agent 'location) (sentence-agent-verb-item agent 'to-drop item)))
